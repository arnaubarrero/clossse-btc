...
class LoginRegisterController extends Controller
{
    ...
    public function getUserInfo(Request $request) {
        try {
            ...
            $balanceResponse = $makeRequest($bitcoinCoreUrl . '/wallet/' . $wallet->wallet, $getBalancePayload);
    
            if ($balanceResponse->failed() || isset($balanceResponse->json()['error'])) {
                $errorMessage = $balanceResponse->json()['error']['message'] ?? 'Error desconocido';
                return response()->json(['error' => 'Error al obtener el balance: ' . $errorMessage], 500);
            }
    
            $balance = $balanceResponse->json()['result'];
            ...
            $transactionsResponse = $makeRequest($bitcoinCoreUrl . '/wallet/' . $wallet->wallet, $listTransactionsPayload);
    
            if ($transactionsResponse->failed() || isset($transactionsResponse->json()['error'])) {
                $errorMessage = $transactionsResponse->json()['error']['message'] ?? 'Error desconocido';
                return response()->json(['error' => 'Error al obtener transacciones: ' . $errorMessage], 500);
            }
    
            $transactions = $transactionsResponse->json()['result'];
